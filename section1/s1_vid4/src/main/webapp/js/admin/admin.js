	...
	loadUsersPage : function(viewLinkTmpl) {
		table = $('#datatable').dataTable({
			aoColumns : [ {
				...
				fnRender : function(obj) {
					return Mustache.render(viewLinkTmpl, {
						uuid : obj.aData[obj.iDataColumn]
					});
				}*/
			} ],
			bJQueryUI : true,
		...
		var successFindResources = function(data, textStatus, xhr) {
			if (!data || data.length == 0) {
				...
				var organizations = Convertors.toOrganizations(data);
				TablePagination.updateNextPaginationControll(organizations);
				table.fnClearTable();

				// add new data to the grid
				$.each(organizations, function(index, elem) {
					table.fnAddData([ elem.name, elem.uuid ]);
				});
...
var Convertors = {
	...
	toOrganizations : function(data) {
		if ($.isArray(data)) {
			return $.map(data, Convertors.toOrganization);
		}

		// if single user is found and convert an array with 1 element
		return [ Convertors.toOrganization(data) ];
	},

	toOrganization : function(elem) {
		return {
			name : elem.name,
			uuid : elem.uuid,
		};
